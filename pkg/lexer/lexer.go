...
func defaultHandler(tokenKind TokenKind, value string) regexHandler {
	return func(lex *lexer, regex *regexp.Regexp) {
		lex.advanceN(len(value))
		lex.push(NewToken(tokenKind, value))
	}
}
...
func Tokenize(source string) []Token {
	...
	for !lex.at_eof() {
		...
		if !matched {
			panic(fmt.Sprintf("Lexer::Error -> unreconized token near %s", lex.remainder()))
		}
	}
